...
object ValidationLogic {
  ...
  def validate(alertPayload: AlertPayload): ValidatedNel[AlertCreationError, Alert] = {
    ...
    val nameValidation: Validated[AlertCreationError, AlertName] = Validated.fromEither(refineV[Name](alertPayload.name)).leftMap( _ => AlertCreationError.InvalidName)
    val tAValidation: Validated[AlertCreationError, TA] = Validated.fromEither(refineV[TAP](alertPayload.tA)).leftMap( _ => AlertCreationError.InvalidTA)
    val tCvalidation: Validated[AlertCreationError, TC] = Validated.fromEither(refineV[TCP](alertPayload.tC)).leftMap(_ => AlertCreationError.InvalidTC)
